    ...
    function (data) {
        ...
        if (data.items != null && data.items.length > 0) {
            for (var i = 0; i < data.items.length; i++) {
                ...
                html += '<tr><td>' + item.UserName + '</td> <td>' + item.UserPoint + '</td><td>' + item.TotalPay + '</td><td>' + item.TotalDraw + '</td><td>' + item.TotalPayMent + '</td> <td>' + item.TotalReturn + '</td><td>' + item.TotalWin + '</td> <td>' + (item.TotalWin - item.TotalPayMent) + '</td></tr>';
            }
        } else {
