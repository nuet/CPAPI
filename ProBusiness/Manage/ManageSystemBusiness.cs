...
namespace ProBusiness.Manage
{
    public class ManageSystemBusiness
    {
        ...
        public static List<Menu> GetMenus() {
            ...
            DataTable dt = SystemDAL.BaseProvider.GetMenus();
            Menu model;
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                if (!string.IsNullOrEmpty(model.PCode))
                    model.PCodeName = GetMenu(model.PCode).Name;
        ...
        public static Menu GetMenu(string menuCode)
        {

            Menu item = new Menu();
            DataTable dt = SystemDAL.BaseProvider.GetMenu(menuCode);
            if (dt.Rows.Count > 0)
            {
                item.FillData(dt.Rows[0]);
                if (!string.IsNullOrEmpty(item.PCode))
                    item.PCodeName = GetMenu(item.PCode).Name;
            }
        ...
        {
            return SystemDAL.BaseProvider.AddMenu(Menu.MenuCode,Menu.Name,Menu.Controller,Menu.View,Menu.PCode,Menu.Sort,Menu.Layer,Menu.Type);
        }

        public static bool UpdateMenu(Menu Menu)
        {
            return SystemDAL.BaseProvider.UpdateMenu(Menu.MenuCode, Menu.Name, Menu.Controller, Menu.View, Menu.Sort);
        }

        public static bool DeleteMenu(string menuCode)
        {
            return SystemDAL.BaseProvider.DeleteMenu(menuCode);
        }
